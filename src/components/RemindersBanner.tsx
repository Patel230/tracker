...
export default function RemindersBanner() {
  const { overdue, complete, error } = useReminders();
  if (!overdue.length && !error) return null;

  return (
    <div className="border-b-[3px] border-brut-ink bg-card px-6 py-2.5">
      {error && (
        <div className="flex items-center gap-2 py-0.5" role="alert">
          <AlertTriangle size={11} strokeWidth={2.5} className="text-destructive" />
          <span className="text-xs font-bold uppercase tracking-wider text-destructive">{error}</span>
        </div>
      )}
      {overdue.length > 0 && (
        <>
